...
function auth(req, res, next) {
  ...
  const initData0 = req.headers['x-telegram-init-data'];
  if (initData0) {
    try {
      const params0 = new URLSearchParams(initData0);
      const userRaw0 = params0.get('user');
      if (userRaw0) {
        const user0 = JSON.parse(userRaw0);
        if (user0 && user0.id) {
          req.telegramUser = { id: String(user0.id), first_name: user0.first_name || 'User', username: user0.username || '' };
          return next();
        }
      }
    } catch(e) {}
...
// TON Connect - Auto verify payment
app.post('/api/budget/ton-verify', auth, async (req, res) => {
  const { amount, boc } = req.body;
  if (!amount || !boc) return res.status(400).json({ error: 'amount and boc required' });
  ...
  data.users[String(user.id)].balance = (data.users[String(user.id)].balance || 0) + Number(amount);
  save(data);

  // Bot se user ko notify karo
  const token = process.env.BOT_TOKEN;
  const adminId = process.env.ADMIN_ID || '7606057137';
  if (token) {
    const msg = `✅ *TON Payment Received!*\n\n👤 User: ${user.firstName} (ID: \`${user.id}\`)\n💎 Amount: ${amount} TON\n\nBalance updated automatically.`;
    const https = require('https');
    const body = JSON.stringify({ chat_id: user.id, text: msg, parse_mode: 'Markdown' });
    const opts = { hostname: 'api.telegram.org', path: `/bot${token}/sendMessage`, method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } };
    https.request(opts, () => {}).end(body);
    // Admin ko bhi batao
    const adminMsg = `💰 *New TON Payment*\n👤 ${user.firstName} (${user.id})\n💎 ${amount} TON added`;
    const body2 = JSON.stringify({ chat_id: adminId, text: adminMsg, parse_mode: 'Markdown' });
    const opts2 = { hostname: 'api.telegram.org', path: `/bot${token}/sendMessage`, method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body2) } };
    https.request(opts2, () => {}).end(body2);
  }
